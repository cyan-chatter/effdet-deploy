    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0:
                regression_losses.append(torch.tensor(0).to(device=device, dtype=torch.float32))
                classification_losses.append(torch.tensor(0).to(device=device, dtype=torch.float32))
                continue
            ...
            alpha_factor = alpha_factor.to(device=device, dtype=torch.float32)
